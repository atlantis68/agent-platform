...
- 2026-07-16 17:31：开始阶段 2，范围限定为工具系统与 MCP v1；本阶段只做 Tool Registry、低风险 HTTP demo 工具、MCP demo tool、工具审计和 SSE 展示，不进入 RAG、长期记忆、Skill 运行时或 CLI。
- 2026-07-16 17:34：任务 1 完成。先新增 `ToolRegistryControllerTest`，确认 `/api/v1/tools` 返回 404 红灯；随后新增 `ToolDefinition`、`ToolRiskLevel`、`ToolRegistryService` 和 `ToolRegistryController`，定向测试输出 `Tests run: 1, Failures: 0, Errors: 0, Skipped: 0`。
- 2026-07-16 17:37：任务 2 完成。Runtime 新增 HTTP 工具和 MCP tool 红灯测试，先失败于缺少 `tool.requested/tool.completed`；随后新增 `runtime/app/entity/tool.py`、`tool_registry.py`、`tool_executor.py`、`mcp_tool_client.py` 并更新 `runtime_service.py`。首次绿灯前发现 Windows 缺少 IANA `tzdata`，将 `mcp_local_time` demo 改为 `Asia/Shanghai` 固定 UTC+8 偏移；Runtime 测试最终输出 `4 passed`。
- 2026-07-16 17:41：任务 3 完成。新增 `runSummaryCountsCompletedToolEvents` 红灯测试，确认 `usage.toolCalls` 仍为 0；随后让 `StoredRun` 从 `tool.completed` 派生工具调用数，让默认 Agent 快照启用 `http_echo` 和 `mcp_local_time`，并把运行台默认输入调整为阶段 2 演示。`AgentRunControllerTest` 输出 `Tests run: 5, Failures: 0, Errors: 0, Skipped: 0`。
- 2026-07-16 17:42：阶段 2 自动化验证通过。`runtime\.venv\Scripts\python -m pytest runtime\tests -q` 输出 `4 passed`；`mvn -f control-plane\pom.xml clean test` 输出 `Tests run: 8, Failures: 0, Errors: 0, Skipped: 0` 和 `BUILD SUCCESS`。
- 2026-07-16 17:45：阶段 2 本机端到端验收通过。使用临时端口 `18091/18092` 启动 Runtime 和控制面，`/health` 与 `/index.html` 均返回 HTTP 200；创建包含 `echo` 和 `mcp 时间` 的 Agent Run 后，审计事件包含两组 `tool.requested/tool.completed`，`toolCompleted=[http_echo,mcp_local_time]`，`usage.toolCalls=2`，SSE `Content-Type=text/event-stream;charset=UTF-8` 且包含工具事件。验收后已停止临时 Runtime 和控制面进程。
- 2026-07-16 17:46：创建阶段 2 成果展示文档 `docs/delivery/phase-2-showcase.md`，更新 README、路线图、任务计划和阶段 2 实施计划；阶段 2 标记为 complete，阶段 3 仅进入 planned 状态，尚未开始编码。
